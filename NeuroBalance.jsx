...
const GlobalStyles = memo(() => (
    <style>{`
        ...
        .report-left { display: flex; flex-direction: column; gap: 2rem; }
        .chart-container { min-width: 300px; }
        .summary-container ul { list-style: none; padding: 0; }
        .summary-container li { margin-bottom: 1rem; font-size: 1.05rem; line-height: 1.5; }
        .summary-container strong { color: var(--primary-neon); font-family: 'Orbitron', sans-serif; }
        .performance-log {
            grid-column: 1 / -1;
            margin-top: 2rem;
        }
        ...
        .log-entry .time { color: var(--goal-neon); }
        .log-entry .event { color: var(--text-color); }
        .log-entry .level { color: var(--distractor-neon); }
        .ai-analysis {
            grid-column: 1 / -1;
            margin-top: 2rem;
        }
        ...
        .disclaimer { margin-top: 2rem; text-align: center; font-weight: bold; color: var(--secondary-neon); }
        .report-buttons { text-align: center; margin-top: 2rem; display: flex; justify-content: center; gap: 1rem; }
...
const sounds = {
    ...
    game_complete: new Howl({ src: ['data:audio/wav;base64,UklGRlAAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YUYAAAAgAAAAKgA/AEgATwBVAFYAVQBRAEwARQBAADsAKgAfABMAAwD/APUA8gDwAO8A7wDsAOsA6wDqAOoA6gDqAOoA6gDqAOoA6g=='] }),
};
...
const generateMaze = (width, height) => {
    ...
    let stack = [];
    let [cx, cy] = [Math.floor(Math.random() * width), Math.floor(Math.random() * height)];
    grid[cy][cx].visited = true; stack.push([cx, cy]);
    while (stack.length > 0) {
        ...
        if (neighbors.length > 0) {
            ...
            if (dir === 'top') { grid[cy][cx].top = false; grid[ny][nx].bottom = false; }
            if (dir === 'right') { grid[cy][cx].right = false; grid[ny][nx].left = false; }
            if (dir === 'bottom') { grid[cy][cx].bottom = false; grid[ny][nx].top = false; }
            if (dir === 'left') { grid[cy][cx].left = false; grid[ny][nx].right = false; }
            grid[ny][nx].visited = true; stack.push([nx, ny]);
        } else { stack.pop(); }
    }
    return grid;
};
...
const findShortestPath = (maze, start, end) => {
    ...
    while (queue.length > 0) {
        ...
        if (x === end.x && y === end.y) return path.length;
        const moves = []; const cell = maze[y][x];
...
const ReportScreen = memo(({ reportData, onRestart }) => {
    ...
    useEffect(() => {
        ...
        if (chartRef.current) {
            ...
            chartInstance = new Chart(ctx, {
                type: 'radar',
                data: {
                    labels: ['Motor Control', 'Cognitive Load', 'Stress Management', 'Behavioral Stability', 'NeuroBalance'],
                    datasets: [{
                        label: 'Performance Score', data: Object.values(reportData.scores),
                        backgroundColor: 'rgba(0, 255, 192, 0.2)', borderColor: 'rgba(0, 255, 192, 1)',
                        pointBackgroundColor: 'rgba(0, 255, 192, 1)', pointBorderColor: '#fff',
                        pointHoverBackgroundColor: '#fff', pointHoverBorderColor: 'rgba(0, 255, 192, 1)'
                    }]
                },
                options: {
                    responsive: true, maintainAspectRatio: true,
                    scales: { r: { beginAtZero: true, max: 100, angleLines: { color: 'rgba(255, 255, 255, 0.2)' }, grid: { color: 'rgba(255, 255, 255, 0.2)' }, pointLabels: { color: '#E0E0E0', font: { size: 12, family: 'Orbitron' } }, ticks: { backdropColor: 'rgba(0,0,0,0)', color: 'rgba(255,255,255,0.5)' } } },
    ...
    const downloadLog = () => {
        const jsonString = `data:text/json;charset=utf-8,${encodeURIComponent(JSON.stringify(reportData.log, null, 2))}`;
        const link = document.createElement("a");
        link.href = jsonString; link.download = "neurobalance_performance_log.json"; link.click();
    };

    return (
        <div className="screen-container report-screen">
            <h1>Performance Report</h1>
            <div className="report-grid">
                <div className="report-left"><div className="chart-container"><canvas ref={chartRef}></canvas></div></div>
                <div className="summary-container">
                    <ul>{reportData.summary.map((item, index) => (<li key={index}><strong>{item.metric}:</strong> ({item.rating}) <br/>{item.text}</li>))}</ul>
                </div>
                <div className="ai-analysis">
                    <h2>✨ AI-Powered Analysis</h2>
                    <p>{reportData.aiAnalysis || "Generating analysis..."}</p>
                </div>
                <div className="performance-log">
                    <h2>Real-Time Performance Log</h2>
                    <div className="log-entries">
                        {reportData.log.length > 0 ? reportData.log.map((entry, index) => (
                            <div key={index} className="log-entry">
                                <span className="time">{new Date(entry.time).toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit', fractionalSecondDigits: 3 })}</span>
                                <span className="event">{entry.event}</span>
                                <span className="level">Level {entry.level}</span>
                            </div>
                        )) : <p>No mistakes recorded. Excellent performance!</p>}
                    </div>
                </div>
            </div>
            <p className="disclaimer">This report is for informational purposes only and is not a substitute for professional medical advice.</p>
            <div className="report-buttons"><button className="btn" onClick={onRestart}>Try Again</button><button className="btn" onClick={downloadLog}>Download Log</button></div>
        </div>
    );
});
...
const GameScreen = ({ onGameComplete }) => {
    const LEVEL_CONFIG = {
        ...
        2: { size: 25, features: ['movingWalls'], numMovingWalls: 4 }
    };
    ...
    const startLevel = useCallback((currentLevel) => {
        ...
        if (config.features.includes('movingWalls')) {
           for (let i = 0; i < config.numMovingWalls; i++) {
                let wallX = Math.floor(Math.random() * (config.size - 2)) + 1;
                let wallY = Math.floor(Math.random() * (config.size - 2)) + 1;
        ...
        metrics.current[currentLevel] = {
            ...
            shortestPath: findShortestPath(newMaze, start, end)
        };
    }, []);
    ...
    useEffect(() => {
        const updateCellSize = () => {
            if (containerRef.current) {
                ...
                cellSize.current = Math.min(width, height) / mazeSize * 0.9;
                setPlayerPos(p => ({...p})); 
            }
        };
    ...
    const logError = useCallback((event) => {
    ...
    useEffect(() => {
        if (playerPos.x === goalPos.x && playerPos.y === goalPos.y) {
            ...
            if (level < Object.keys(LEVEL_CONFIG).length) {
                sounds.level_complete.play(); setLevel(l => l + 1); setTimer(0);
            } else {
    ...
    const cSize = Math.max(5, cellSize.current);
    const mazeSize = LEVEL_CONFIG[level].size;
    const features = LEVEL_CONFIG[level].features;
    
    const mazeContainerStyle = useMemo(() => {
        const style = {
        ...
        if(applySpotlight) {
            ...
            style['-webkit-mask-image'] = gradientStyle;
            style['mask-image'] = gradientStyle;
        }
        return style;
    }, [cSize, mazeSize, features, isBlinking, playerPos]);

    const mazeGridWrapperStyle = useMemo(() => {
        return {
            display: 'grid', 
    ...
    return (
        <div className="game-screen" ref={containerRef}>
            <div className="game-ui"><span>Level: {level}</span><span>Time: {timer}s</span></div>
...
// --- MAIN APP COMPONENT ---
const NeuroBalanceMaze = () => {
    const [gameState, setGameState] = useState('start');
    const [finalReportData, setFinalReportData] = useState(null);

    const handleGameComplete = useCallback(async (fullMetrics, log) => {
        setGameState('report');
        const totalCollisions = Object.values(fullMetrics).reduce((acc, curr) => acc + curr.wallCollisions, 0);
        const totalShortest = Object.values(fullMetrics).reduce((acc, curr) => acc + curr.shortestPath, 0);
        const totalMoves = Object.values(fullMetrics).reduce((acc, curr) => acc + curr.moves, 0);
        ...
        const totalSharpTurns = Object.values(fullMetrics).reduce((acc, curr) => acc + curr.sharpTurns, 0);
        ...
        const scores = {
            ...
            environmentalStress: Math.round(stressScore), behavioralStability: Math.round(stabilityScore),
        ...
        const summary = [
            ...
            { metric: "Stress Management", rating: getRating(scores.environmentalStress), text: "Gauges adaptability to dynamic and unpredictable challenges, such as moving walls." },
        ...
        setFinalReportData({ scores, log, summary, aiAnalysis: "Analyzing your performance..." });
        ...
        try {
            ...
            if (!apiKey) {
                setFinalReportData(prevData => ({ ...prevData, aiAnalysis: "AI Analysis is disabled. Please configure your Google AI API key in a .env file to see your personalized report." }));
                return;
            }
            ...
            setFinalReportData(prevData => ({ ...prevData, aiAnalysis: analysisText }));
        } catch (error) {
            console.error("AI Analysis Error:", error);
            setFinalReportData(prevData => ({ ...prevData, aiAnalysis: "Could not generate AI analysis at this time. Please check your connection or try again later." }));
        }
    }, []);
    
    const handleRestart = () => { setFinalReportData(null); setGameState('start'); };
    ...
    return (
        <div className="app-container">
            ...
            {gameState === 'report' && finalReportData && <ReportScreen reportData={finalReportData} onRestart={handleRestart} />}
        </div>
    );
};

export default NeuroBalanceMaze;